--[[
local food_order = { "a Pepperoni with Mushrooms", "a Cantonese Duck", "some Buffalo Wings" }
local food_share = { "Chinese", "Mexican", "Sushi", "Pizza", "Chicken Wings" }
local appointment_type = { "Dermatologist", "Dentist", "Doctor's", "Chiropractor", "Court", "Vet", "Physiotherapy" }
local game_name = { "Deep Rock", "GTA", "Garry's Mod", "Killing Floor", "Lethal Company", "Raid", "VRChat", "Chicken Horse", "Worms", "Euro Truck", "TF2", "Left 4 Dead", "Crime Boss" }
]]
...
Hooks:PreHook(HintManager, "_show_hint", "FunnyHints_main", function(self, id, time, only_sync, params)
	...
	if Global.custom_hints_main[string_id] then
		local text = table.random(Global.custom_hints_main[string_id])
